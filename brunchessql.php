...
while ( $i<= 1) {
	foreach ($rows as $row) {
		...
		$ct=$row['vip'];
		$id=$row['vip'];
		$sql = "INSERT INTO `orderr` (`idprun`, `brunches` ,`idname`,`itemid`, `name`, `count` , `date`) VALUES ('$idprun' , '$brunches' ,'$c','$id', '$_POST[$t]', '$_POST[$ct]' , '$date')";
$conn->exec($sql);
	}
	$i++;
}
...
$sql = "INSERT INTO `special` (`brunches`,`size`, `count`, `describe`,`imge`, `date`) 
VALUES ('$brunches' ,'$si', '$c', '$d','$imge','$date')";
$conn->exec($sql);


$sql = "DELETE FROM orderr WHERE count=0";
$conn->exec($sql);
$sql = "DELETE FROM special WHERE count=0";
$conn->exec($sql);
